...
- Компоненты:
  ...
  - `AdminUserDto` (`users/dto/admin-user.dto.ts`) — Swagger-DTO ответа: `id`, `login`, `role`, `provider`, `createdAt`, `updatedAt`
...
- `GET /admin/users` — `src/admin/users/admin-users.controller.ts` (#59) — защищён `JwtAuthGuard` + `RolesGuard(ADMIN)`, пагинированный список пользователей (`{ items, meta }`), не отдаёт `passwordHash`/`googleId`

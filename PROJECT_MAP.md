...
- Компоненты:
  ...
  - `AuthController` (`auth.controller.ts`) — `GET /auth/me` (защищён), `POST /auth/logout` (публичный), `POST /auth/register`, `POST /auth/login` (публичные, локальный логин/пароль; throttled — см. ниже), `POST /auth/change-password` (защищён `JwtAuthGuard`, #50) — смена пароля текущего пользователя, `403` при `user.provider === 'google'` (#57, документировано в Swagger)
  - `LocalAuthService` (`local-auth.service.ts`) — `register()` (bcrypt-хэш пароля, `User`+`Profile`+`Settings` одной вложенной записью — атомарно без явного `$transaction`, `409` при занятом `login`), `validateCredentials()` (поиск + `bcrypt.compare`, единый `401` независимо от того, что именно неверно), `changePassword(userId, dto)` (#50, #57) — находит `User` по `id`; если `user.provider === 'google'` — сразу `403 ForbiddenException` («Смена пароля недоступна: вход в этот аккаунт выполняется через Google», проверка раньше остальных, до `passwordHash`/`bcrypt.compare`); иначе тот же паттерн, что раньше: `401` если пользователя нет или у него нет `passwordHash`, `bcrypt.compare(currentPassword, ...)`, `401` при несовпадении, иначе `bcrypt.hash(newPassword, BCRYPT_SALT_ROUNDS)` и `prisma.user.update`
  - `ChangePasswordDto` (`dto/change-password.dto.ts`, #50) — `currentPassword` (`@IsString()`), `newPassword` (`@IsString() @MinLength(8) @MaxLength(72)`, те же ограничения, что `RegisterDto.password`)
  - `GoogleAuthService` (`google-auth.service.ts`) — `authenticate()`: верификация ID-токена через `google-auth-library` (`OAuth2Client.verifyIdToken`, `audience = GOOGLE_CLIENT_ID`; без Client Secret — чистая ID-token-проверка, не authorization-code флоу), поиск по `googleId` → авто-связка по `Profile.email` (только если `email_verified === true`, `provider` при этом **не** трогается — у аккаунта уже есть свой локальный пароль) → создание нового `User`+`Profile`+`Settings` с `provider: 'google'` (#57; `login = googleId`, т.к. `User.login` обязателен, а у Google-пользователя своего login нет и он с ним не взаимодействует), `Profile.name`/`Profile.avatarUrl` заполняются из `payload.name`/`payload.picture` (ФИО и фото из Google-токена, если есть — не запрашиваются отдельным scope, приходят вместе с базовым `profile`/`email`); дальше оба поля обычные редактируемые поля в БД — пользователь может сменить их через `PATCH /profile`/`PATCH /profile/avatar` (#49), как у любого локального аккаунта. Любая ошибка верификации токена — единый `401`.
  ...
  - `UserWithProfile` (`types/user-with-profile.type.ts`) — `UserEntity & { profile: { email: string | null; name: string | null; avatarUrl: string | null } | null }`, переиспользуется `LocalAuthService`/`GoogleAuthService`; `LocalAuthService`/`GoogleAuthService`/`AuthController.me()` оборачивают результат `prisma.user.*` в `UserEntity` перед возвратом наружу (защита от случайной утечки `passwordHash`/`googleId`, даже если будущий код вернёт сущность напрямую в обход `UserMeDto`)
...
- Компоненты:
  - `AdminUsersService` (`users/admin-users.service.ts`) — `findAll(query)` (пагинация через `AdminUsersQueryDto extends PaginationQueryDto`/`buildPaginationMeta`; `#61` добавил опциональные фильтры `login` (`{ contains }`, без `mode: 'insensitive'` — не поддерживается Prisma на MySQL, коллейшн БД сам обычно регистронезависим) и `role` (точное совпадение), оба комбинируются через spread в `where`, передаётся и в `findMany`, и в `count`), `findOne(id)` (`#61`, новый) — `prisma.user.findUnique` с `include: { profile, gameAccounts, socialLinks }`, `404 NotFoundException('Пользователь не найден')` если нет записи, иначе вручную собирает `AdminUserDetailDto` (email/name/avatarUrl из `profile` с фолбэком в `null`, gameAccounts/socialLinks как есть) — намеренно **не** через `UserEntity`, т.к. это plain-объект без `passwordHash`/`googleId` в принципе; `updateRole(currentUserId, id, role)`/`remove(currentUserId, id)` — оба сначала проверяют `currentUserId === id` (`403 ForbiddenException`, нельзя изменить роль/удалить самого себя), затем существование записи (`404 NotFoundException`); эти три метода оборачивают результат `prisma.user.*` в `UserEntity` (`src/auth/entities/user.entity.ts`) — первый в проекте случай, когда `UserEntity` возвращается из HTTP-ответа напрямую (не через промежуточный DTO, как в `AuthController`), поэтому именно здесь реально проверяется (не только предполагается), что `ClassSerializerInterceptor` вправду вырезает `passwordHash`/`googleId` в ответе — см. `admin-users.controller.spec.ts` (`describe('AdminUsersController (serialization)')`, отдельный `TestingModule` с явно подключенным `ClassSerializerInterceptor`, единственный такой тест в проекте)
  - `AdminUsersController` (`users/admin-users.controller.ts`) — `@Controller('admin/users')`, `@UseGuards(JwtAuthGuard, RolesGuard)` + `@Roles(Role.ADMIN)` на уровне контроллера; `GET /:id` (`#61`, новый) вызывает `findOne`
  - `UpdateUserRoleDto` (`users/dto/update-user-role.dto.ts`) — `role` через `@IsIn([Role.USER, Role.ADMIN])` (не `@IsEnum(Role)`) — `MODERATOR` или любое другое значение даёт `400`; валидация покрыта отдельным `update-user-role.dto.spec.ts` (конвенция проекта — валидация DTO тестируется напрямую через `class-validator`, а не через `ValidationPipe` в контроллерном тесте, т.к. `ValidationPipe` подключается только в `main.ts`, не в тестовом `TestingModule`)
  - `AdminUsersQueryDto` (`users/dto/admin-users-query.dto.ts`, `#61`) — `extends PaginationQueryDto`, добавляет `login?: string` (`@IsOptional() @IsString()`) и `role?: Role` (`@IsOptional() @IsEnum(Role)` — здесь допустимы все три значения enum, в отличие от `UpdateUserRoleDto`, где только `USER`/`ADMIN`)
  - `AdminUserDto` (`users/dto/admin-user.dto.ts`) — Swagger-DTO ответа списка: `id`, `login`, `role`, `provider`, `createdAt`, `updatedAt`
  - `AdminUserDetailDto` (`users/dto/admin-user-detail.dto.ts`, `#61`) — `extends AdminUserDto`, добавляет `email`/`name`/`avatarUrl` (nullable, из `Profile`) и `gameAccounts: GameAccountDto[]`/`socialLinks: SocialLinkDto[]` (переиспользует существующие DTO из `ProfileModule`, не дублирует)
...
- `GET /auth/me` — `src/auth/auth.controller.ts` — защищён `JwtAuthGuard`, возвращает `{ id, login, role, email, name, avatarUrl }` текущей сессии (`name`/`avatarUrl` из `Profile`, добавлены как блокер AC stream.Front#64 — нужны фронту в одном запросе `initializeAuth`, без второго вызова `GET /profile`, иначе неизбежно мигание аватара на hard refresh), `401` без/с невалидной cookie
...
- `GET /admin/users` — `src/admin/users/admin-users.controller.ts` (#59, фильтры `login`/`role` добавлены в #61) — защищён `JwtAuthGuard` + `RolesGuard(ADMIN)`, пагинированный список пользователей (`{ items, meta }`), опциональные `login` (contains, без учёта регистра) и `role` (точное совпадение, комбинируются), не отдаёт `passwordHash`/`googleId`
- `GET /admin/users/:id` — `src/admin/users/admin-users.controller.ts` (#61) — защищён `JwtAuthGuard` + `RolesGuard(ADMIN)`, полная карточка пользователя (`id`/`login`/`role`/`provider`/`createdAt`/`updatedAt`/`email`/`name`/`avatarUrl`/`gameAccounts[]`/`socialLinks[]`), `404` если `:id` не существует, не отдаёт `passwordHash`/`googleId`; read-only (без редактирования вложенных данных)
...
- `DELETE /admin/users/:id` — `src/admin/users/admin-users.controller.ts` (#59) — защищён `JwtAuthGuard` + `RolesGuard(ADMIN)`, жёсткое удаление пользователя (каскадно удаляет `Profile`/`Settings`/`GameAccount`/`SocialLink`); `403` при попытке удалить себя, `404` если `:id` не существует

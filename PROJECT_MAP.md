...
- Компоненты:
  ...
  - `AuthController` (`auth.controller.ts`) — `GET /auth/me` (защищён), `POST /auth/logout` (публичный), `POST /auth/register`, `POST /auth/login` (публичные, локальный логин/пароль; throttled — см. ниже), `POST /auth/change-password` (защищён `JwtAuthGuard`, #50) — смена пароля текущего пользователя
  - `LocalAuthService` (`local-auth.service.ts`) — `register()` (bcrypt-хэш пароля, `User`+`Profile`+`Settings` одной вложенной записью — атомарно без явного `$transaction`, `409` при занятом `login`), `validateCredentials()` (поиск + `bcrypt.compare`, единый `401` независимо от того, что именно неверно), `changePassword(userId, dto)` (#50) — тот же паттерн: находит `User` по `id`, `401` если пользователя нет или у него нет `passwordHash` (например Google-only), `bcrypt.compare(currentPassword, ...)`, `401` при несовпадении, иначе `bcrypt.hash(newPassword, BCRYPT_SALT_ROUNDS)` и `prisma.user.update`
